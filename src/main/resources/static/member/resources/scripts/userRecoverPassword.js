...
contactAuthRequestButton.addEventListener('click',()=>{
    if (recoverEmailForm['email'].value === '') {
        alert('이메일 주소를 입력해 주세요.');
        recoverEmailForm['email'].focus();
        return;
    }

    if (!new RegExp('^(?=.{7,50})([\\da-zA-Z_.]{4,})@([\\da-z\\-]{2,}\\.)?([\\da-z\\-]{2,})\\.([a-z]{2,10})(\\.[a-z]{2})?$').test(recoverEmailForm['email'].value)) {
    ...
    xhr.open('GET', `./userRecoverPasswordAuth?email=${recoverEmailForm['email'].value}&contact=${recoverEmailForm['contact'].value}`);
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                switch (responseJson['result']) {
                    case 'success':
                        ...
                        recoverEmailForm['email'].setAttribute('disabled', 'disabled');
                        ...
                        break;
                    default:
                        alert('입력하신 이메일과 연락처가 일치하는 회원이 없습니다.');
                        recoverEmailForm['contact'].focus();
                }
            } else {
